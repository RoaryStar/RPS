...
typedef char g_res;     // one of G_PONE, G_PTWO, G_TIE
typedef int toss;      // one of ROCK, PAPER, or SCISSORS

int wins_against (toss p1, toss p2)
{
    return (3+p2-p1)%3 == 1;
}
// winner : toss, toss -> g_res
// Results the winer in a game where the lhs player throws p1 and
// the rhs player throws p2.
g_res winner (toss p1, toss p2)
{
    if (wins_against (p1, p2)) return G_PONE;
    if (wins_against (p2, p1)) return G_PTWO;
    return G_TIE;
}
...
// of 10.
double* adj_ent (histrecord* h, double* out)
{
#define ASYMPTOT 10
...
double* aggregate (toss* hist, histrecord* h, double* res, double* work, int n)
{
    if (n == 0)
    {
        adj_ent(h, work);
...
histrecord* make_hr_recurse (histrecord* h, int n, int d, double s)
{
    if (d > 0)
    {
        h->n = n;
        h->inner = (histrecord*) malloc (sizeof (histrecord)
                                         * n);
        for (int i = 0; i < n; ++i)
...
histrecord* force_entropy_calc (histrecord* h, int d)
{
    if (d == 1)
    {
...
p_champion* make_champion (p_champion* p, int n)
{
    p->hist_len = n * 2;
...
// next move.
toss next_throw_champion (p_champion* c)
{
...
toss choose_champion (pdata d)
{
    next_throw_champion((p_champion*) d);
}

histrecord* update_rec (histrecord* rec, toss op_play, double mult)
{
    for (int i = 0; i < 3; ++i)
    {
...
histrecord* rec_update (histrecord* rec, toss* hist, toss op_play, double mult, int d)
{
    if (d == 1)
...
void update_champion (p_champion* c, toss mp, toss op)
{
    rec_update(c->coeffs,
               c->history + c->h_index,
               op, 1.0, c->hist_len);
